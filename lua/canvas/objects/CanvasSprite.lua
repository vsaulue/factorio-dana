...
-- Class used to draw a sprite on a Canvas.
--
-- RO fields: all from AbstractCanvasObject.
--
local CanvasSprite = ErrorOnInvalidRead.new{
    -- Creates a new CanvasSprite, and its associated rendering id.
    --
    -- Args:
    -- * initData: Table passed to the rendering API to generate the id.
    --
    -- Returns: The new CanvasSprite object.
    --
    makeFromInitData = function(initData)
        return AbstractCanvasObject.new{
            id = rendering.draw_sprite(initData),
            type = "sprite",
        }
    end,
